...
export class WishListService {
  ...
  addtowishlist(mId: string, isbn: string, token){
    ...
    return this.http.get("https://webtechsolutionsapi.azurewebsites.net/lims3/addtowishlist/" + mId + "/" + isbn)
  ...
  getBookDetails(mId: string, token): Observable<Data[]> {
    ...
    let headers = new Headers({ 'Authorization': 'Bearer ' + token });
    let options = new RequestOptions({ headers: headers });
    return this.http.get("https://webtechsolutionsapi.azurewebsites.net/lims3/getisbndetails/" + mId, options)
      .map((res: Response) => res.json());
  }
  getWishBookDetails(mId: string, token): Observable<Data[]> {
    ...
    let headers = new Headers({ 'Authorization': 'Bearer ' + token });
    let options = new RequestOptions({ headers: headers });
    return this.http.get("https://webtechsolutionsapi.azurewebsites.net/lims3/getwishlist/" + mId, options)
      .map((res: Response) => res.json());
  }
  removeBook(isbn:string,mid:string,token):Observable<Data[]> {
    ...
    let headers = new Headers({ 'Authorization': 'Bearer ' + token });
    let options = new RequestOptions({ headers: headers });
    return this.http.get("https://webtechsolutionsapi.azurewebsites.net/lims3/removewishlist/" + mid + "/" + isbn, options)
      .map((res: Response) => res.json());
  }
