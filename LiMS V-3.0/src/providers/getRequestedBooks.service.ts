...
export class GetRequestedBooks {
 token;
  constructor(public http: Http, public adalService: AdalService) {
    ...
    return this.http.get('https://webtechsolutionsapi.azurewebsites.net/lims3/getAllRequestBooksHistory',options)
  ...
  getReturnRequestedBooks():Observable<Data[]>{
     ...
     let headers = new Headers({ 'Authorization': 'Bearer ' + this.token });
      let options = new RequestOptions({ headers: headers });
    return this.http.get('https://webtechsolutionsapi.azurewebsites.net/lims3/getAllReturnRequestBooksHistory',options)
     .map((res:Response)  => res.json());
  }
  ...
  acceptReturn(mid:string,bookId:string){
     ...
     let headers = new Headers({ 'Authorization': 'Bearer ' + this.token });
      let options = new RequestOptions({ headers: headers });
    return this.http.get('https://webtechsolutionsapi.azurewebsites.net/lims3/return/'+mid+'/'+bookId,options)
  }
}