...
export class HomeBodyService {
  ...
  getMyBookIDs(id: string, token): Observable<data[]> {
    ...
    console.log("hiee")
    return this.http.get("https://webtechsolutionsapi.azurewebsites.net/lims3/getmyMobileAssetBookIds/" + id, options)
      .map((res: Response) => res.json());
  }
  getMyBook(id: string, name: string, token): Observable<data[]> {
    let headers = new Headers({ 'Authorization': 'Bearer ' + token });
    let options = new RequestOptions({ headers: headers });
    return this.http.get("https://webtechsolutionsapi.azurewebsites.net/lims3/getmyAsset/" + id + "/" + name, options)
      .map((res: Response) => res.json());
  }
  returnRequest(mid: string, bookId: string, token): Observable<data[]> {
    let headers = new Headers({ 'Authorization': 'Bearer ' + token });
    let options = new RequestOptions({ headers: headers });
    return this.http.get("https://webtechsolutionsapi.azurewebsites.net/lims3/returnRequest/" + mid + "/" + bookId, options)
      .map((res: Response) => res.json());
  }
  renewBook(mid: string, bookId: string, token){
    let headers = new Headers({ 'Authorization': 'Bearer ' + token });
    let options = new RequestOptions({ headers: headers });
    return this.http.get("https://webtechsolutionsapi.azurewebsites.net/lims3/renewBook/" + bookId + "/" + mid, options)
      .map((res: Response) => res);
  }
  rating(mid: string, isbn: string, value: number, token): Observable<data> {
    let headers = new Headers({ 'Authorization': 'Bearer ' + token });
    let options = new RequestOptions({ headers: headers });
    return this.http.get("https://webtechsolutionsapi.azurewebsites.net/lims3/rating/" + mid + "/" + isbn + "/" + value, options)
      .map((res: Response) => res.json());
  }
  ...
  createProfile(mid: string, name: string, email: string, token) {
    let headers = new Headers({ 'Authorization': 'Bearer ' + token });
    let options = new RequestOptions({ headers: headers });
    return this.http.get("https://webtechsolutionsapi.azurewebsites.net/lims3/createProfile/" + mid + "/" + name + "/" + email, options)
      .map((res: Response) => res);
  }
cancelRequset(mid:string,isbn:string,token){
    ...
    return this.http.get("https://webtechsolutionsapi.azurewebsites.net/lims3/declineRequest/" + mid + "/" + isbn , options)
      .map((res: Response) => res); 
  }
