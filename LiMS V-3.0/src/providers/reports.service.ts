...
export class GetReportsService {
 token;
  constructor(public http: Http, public adalService: AdalService) {
    ...
    return this.http.get('https://webtechsolutionsapi.azurewebsites.net/lims3/getAllIssuedBooksHistory',options)
  ...
  getAllReturnedHistory():Observable<Data[]>{
     ...
     let headers = new Headers({ 'Authorization': 'Bearer ' + this.token });
      let options = new RequestOptions({ headers: headers });
    return this.http.get('https://webtechsolutionsapi.azurewebsites.net/lims3/getAllReturnedBooksHistory',options)
     .map((res:Response)  => res.json());
  }
}